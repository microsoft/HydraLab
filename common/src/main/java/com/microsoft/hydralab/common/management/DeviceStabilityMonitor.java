...
public class DeviceStabilityMonitor {
    ...
    public void stabilityCheck(DeviceInfo deviceInfo, DeviceManager.MobileDeviceState state, String deviceBehaviour) {
        if (deviceInfo == null) {
        ...
        if (!StringUtils.isEmpty(deviceBehaviour)) {
            if (state == DeviceManager.MobileDeviceState.OTHER) {
                ...
                ThreadPoolUtil.TIMER_EXECUTOR.schedule(new Runnable() {
                    ...
                    public void run() {
                        if (isWaitingForConverting.compareAndSet(true, false)) {
                            ...
                            stabilityCheck(deviceInfo, DeviceManager.MobileDeviceState.OTHER, "");
                        }
                    }
        ...
        if (monitor != null) {
            if (DeviceInfo.ONLINE.equals(deviceInfo.getStatus())) {
                monitor.onDeviceConnected(deviceInfo);
            } else {
                monitor.onDeviceInactive(deviceInfo);
            }
        }
    }
