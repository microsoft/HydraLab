...
function Download-and-Install-AppxPackage {
    [CmdletBinding()]
    param (
      [string]$Uri,
      [string]$Path
    ...
    process {
        ...
        foreach($url in $DownloadLinks){
            ...
            if ($FileName.contains("AppleInc.iTunes_")) {
                Write-Host "Downloading $FileName from $url to $FilePath"
                Write-Host
    
                [System.IO.File]::WriteAllBytes($FilePath, $FileRequest.content)
    
                Write-Host "Installing $FileName. This may take some time. Please wait..."
                Write-Host
...
if ((New-Object Security.Principal.WindowsPrincipal $([Security.Principal.WindowsIdentity]::GetCurrent())).IsInRole([Security.Principal.WindowsBuiltinRole]::Administrator)) {
    ...
    Write-Host '4. Install iTunes and tidevice'
    Write-Host 

    $iTunes_Store_Url = 'https://apps.microsoft.com/store/detail/itunes/9PB2MZ1ZMB1S'

    Download-and-Install-AppxPackage "https://apps.microsoft.com/store/detail/itunes/9PB2MZ1ZMB1S" $destination

    $Arguments = "install -U `"tidevice[openssl]`""

    try {
        Write-Host "Installing tidevice. This may take some time. Please wait..."
        $proc = Start-Process -FilePath "$(Join-Path $targetDir "Scripts\pip.exe")" -ArgumentList $Arguments -Wait -PassThru
        $proc.waitForExit()
        Write-Host 'Installation Done.'
        Write-Host
    } catch [exception] {
    ...
    Write-Host '7. Install WinAppDriver'
    Write-Host
