...
import { setSessionDetails, quitSession, setSwitching } from './Inspector';
...
export const SET_BROWSER_DRIVER = 'SET_BROWSER_DRIVER';
export const SET_USE_EXISTING_WINDOW = 'SET_USE_EXISTING_WINDOW';
...
export const WRITE_BACK_SERVER_ARGS = 'WRITE_BACK_SERVER_ARGS';
...
// 60s default for new session
export const NEW_SESSION_TIMEOUT_SEC = 60;
...
export const POWER_SHELL_PATH = 'C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe';
...
export function setApp(testApp) {
  return (dispatch) => {
    dispatch({ type: SET_APP, testApp })
  }
}

export function setUrl(testUrl) {
  return (dispatch) => {
    dispatch({ type: SET_URL, testUrl })
  }
}

export function setBrowerDriver(browserDriver) {
  return (dispatch) => {
    dispatch({ type: SET_BROWSER_DRIVER, browserDriver });
  }
}

export function setUseExistingWindow(isUseExistingWindow) {
  return (dispatch) => {
    dispatch({ type: SET_USE_EXISTING_WINDOW, isUseExistingWindow })
  }
}
...
function proxyWda (udid) {
  let process = require('child_process');
  let portRelayCommand = 'tidevice -u ' + udid + ' relay 8100 8100';
  let startWDACommand = 'tidevice -u ' + udid + '  xctest --bundle_id com.microsoft.wdar.xctrunner';
  let killCommand1 = POWER_SHELL_PATH + " -Command " + "\"Get-WmiObject Win32_Process  -Filter \\\"name like '%tidevice%' and CommandLine like '%" + portRelayCommand.replace(" ", "%") + "%'\\\" | Select-Object ProcessId -OutVariable pids; if(-not $pids -eq '' ) {stop-process -id $pids.ProcessId}\"";
  let killCommand2 = POWER_SHELL_PATH + " -Command " + "\"Get-WmiObject Win32_Process  -Filter \\\"name like '%tidevice%' and CommandLine like '%" + startWDACommand.replace(" ", "%") + "%'\\\" | Select-Object ProcessId -OutVariable pids; if(-not $pids -eq '' ) {stop-process -id $pids.ProcessId}\"";
  process.execSync(killCommand1);
  process.execSync(killCommand2);
  process.exec(portRelayCommand);
  process.exec(startWDACommand);
}

export function getAppTopLevelWindowCaps(appFamilyName, desiredCapabilities) {
  let process = require('child_process');
  let processInfo = process.execSync(POWER_SHELL_PATH + ' -Command build/WindowsAppIdToHandle.ps1 ' + appFamilyName);
  processInfo = parseInt(processInfo).toString(16);
  let newDesiredCapabilities = Object.assign([], desiredCapabilities);
  newDesiredCapabilities = omit(newDesiredCapabilities, ['appium:app']);
  newDesiredCapabilities['appium:appTopLevelWindow'] = processInfo;
  return newDesiredCapabilities;
}

export function getBrowserCaps(desiredCapabilities) {
  let process = require('child_process');
  let processInfo = process.execSync(POWER_SHELL_PATH + " -Command " + "\"(Get-Process | where {$_.mainWindowTitle -and $_.mainWindowHandle -ne 0 -and $_.Name -eq 'msedge'} | Select mainWindowHandle).mainWindowHandle\"");
  console.log("processInfo" + processInfo);
  processInfo = parseInt(processInfo).toString(16);
  let newDesiredCapabilities = Object.assign([], desiredCapabilities);
  newDesiredCapabilities = omit(newDesiredCapabilities, ['appium:app']);
  newDesiredCapabilities['platformName'] = 'windows';
  newDesiredCapabilities['appium:appTopLevelWindow'] = processInfo;
  return newDesiredCapabilities;
}
 ...
 */
export function newSession(caps, attachSessId = null, isRestart = false, startApp = null, testUrl = null) {
  return async (dispatch, getState) => {
    let session = getState().session;
    let testApp = startApp ? startApp : session.testApp;
    console.log('new session ', caps[0].value + ' app =' + testApp);


    //for iOS device, proxy WDA
    if (caps[0].value == 'ios') {
      let udid = caps[1].value;
      let process = require('child_process');
      if (testApp) {
        console.log('launch app for iOS');
        process.exec('tidevice -u ' + udid + ' launch ' + testApp);
      }
      console.log('Proxy WDA for iOS, udid=' + udid + ' app=' + testApp);
      proxyWda(udid);
    }
    ...
    let driver = null;
    let browserDriver = session.browserDriver;
    try {
      if (attachSessId) {
        ...
        console.log('session.isUseExistingWindow');
        console.log(session.isUseExistingWindow);
        if (session.isUseExistingWindow && caps[0].value === 'windows') {
          // Windows Fulltrust app
          driver = await Web2Driver.remote(serverOpts, getAppTopLevelWindowCaps(testApp, desiredCapabilities));
        } else if (caps[0].value === 'browser') {
          // Browser
          if (!browserDriver) {
            const edgeDriverPaths = await installDriver();
            console.log(edgeDriverPaths)
            const edgeOptions = new edge.Options();
            edgeOptions.setBinaryPath(edgeDriverPaths.browserPath);
            console.log(edgeOptions)
            browserDriver = await new Builder()
              .forBrowser(Browser.EDGE)
              .setEdgeOptions(edgeOptions)
              .setEdgeService(new edge.ServiceBuilder(edgeDriverPaths.driverPath))
              .build();
            dispatch({ type: SET_BROWSER_DRIVER, browserDriver });
          }
          if (testUrl) {
            await browserDriver.get(testUrl);
          }                    
          driver = await Web2Driver.remote(serverOpts, getBrowserCaps(desiredCapabilities));
        } else {
      ...
      if (caps[0].value === 'windows') {
        // for windows full trust apps, try starting session by app top level window
        console.log('Try starting session by app top level window!');
        try {
          driver = await Web2Driver.remote(serverOpts, getAppTopLevelWindowCaps(testApp, desiredCapabilities));
        } catch (e) {
          showError(err, null, 0);
          const setSwitchAction = setSwitching(false);
          setSwitchAction(dispatch);
          return;
        }
      } else {
        showError(err, null, 0);
    ...
    // code boilerplate
    let isUseExistingWindow = session.isUseExistingWindow;
    ...
    dispatch({ type: SET_APP, testApp: null });
    dispatch({ type: SET_URL, testUrl: null });
    if (!isRestart) {
...
export function writeBackServerArgs (serverArgs) {
  return async (dispatch) => {
    dispatch({type: WRITE_BACK_SERVER_ARGS, serverArgs});
  };
}
