...
public class UserTeamManagementServiceTest {
    ...
    @Before
    public void setUp() {
        UserTeamRelation userTeamRelation = new UserTeamRelation("teamId", "mailAddress", true);
        Mockito.when(userTeamRelationRepository.findByMailAddressAndTeamId(anyString(), anyString())).thenReturn(Optional.of(userTeamRelation));
    }
