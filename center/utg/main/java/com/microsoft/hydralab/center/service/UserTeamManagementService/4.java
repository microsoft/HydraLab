...
public void testCheckUserExistenceWithTeam_Exist() {
 List<SysUser> users = new ArrayList<>(); SysUser user1 = new SysUser(); user1.setMailAddress("user1@example.com"); users.add(user1); Mockito.when(userTeamManagementService.queryUsersByTeam(teamId)).thenReturn(users); boolean result = userTeamManagementService.checkUserExistenceWithTeam(requestor, teamId); assertTrue(result); 
}

@Test
public void testCheckUserExistenceWithTeam_NotExist() {
 List<SysUser> users = new ArrayList<>(); Mockito.when(userTeamManagementService.queryUsersByTeam(teamId)).thenReturn(users); boolean result = userTeamManagementService.checkUserExistenceWithTeam(requestor, teamId); assertFalse(result); 
}
