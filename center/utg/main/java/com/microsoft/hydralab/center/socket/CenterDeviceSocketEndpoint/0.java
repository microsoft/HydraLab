...
@Before
public void setUp() {
 MockitoAnnotations.initMocks(this); centerDeviceSocketEndpoint = new CenterDeviceSocketEndpoint(); SpringApplicationListener.getApplicationContext().getAutowireCapableBeanFactory().autowireBean(centerDeviceSocketEndpoint); 
}

@Test
public void testOnOpen() {
 centerDeviceSocketEndpoint.onOpen(session); verify(deviceAgentManagementService).onOpen(session); 
}
