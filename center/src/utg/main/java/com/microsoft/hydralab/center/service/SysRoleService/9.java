...
public void testGetOrCreateDefaultRole() {
 String roleName = "Test Role"; int authLevel = 1; SysRole expectedRole = new SysRole(); expectedRole.setRoleName(roleName); expectedRole.setAuthLevel(authLevel); expectedRole.setCreateTime(new Date()); expectedRole.setUpdateTime(new Date()); when(sysRoleRepository.findByRoleName(roleName)).thenReturn(Optional.empty()); when(sysRoleRepository.save(Mockito.any(SysRole.class))).thenReturn(expectedRole); SysRole actualRole = sysRoleService.getOrCreateDefaultRole(roleName, authLevel); assertEquals(expectedRole, actualRole); 
}
