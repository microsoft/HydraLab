...
@Before
public void setUp() {
 MockitoAnnotations.initMocks(this); userTeamManagementService = new UserTeamManagementService(); userTeamManagementService.setUserTeamRelationRepository(userTeamRelationRepository); userTeamManagementService.setSysTeamService(sysTeamService); userTeamManagementService.setSysUserService(sysUserService); userTeamManagementService.setSysRoleService(sysRoleService); 
}

@Test
public void testAddUserTeamRelation() {
 String teamId = "team1"; SysUser user = new SysUser(); user.setMailAddress("user1@example.com"); boolean isTeamAdmin = true; SysTeam team = new SysTeam(); team.setTeamId(teamId); when(sysTeamService.queryTeamById(teamId)).thenReturn(team); UserTeamRelation result = userTeamManagementService.addUserTeamRelation(teamId, user, isTeamAdmin); assertEquals(teamId, result.getTeamId()); assertEquals(user.getMailAddress(), result.getMailAddress()); assertEquals(isTeamAdmin, result.isTeamAdmin()); verify(userTeamRelationRepository, times(1)).save(result); 
}
