...
@Before
public void setUp() {
 taskQueue = new LinkedList<>(); isRunning = new AtomicBoolean(false); 
}

@Test
public void testRunTask() {
 TestTaskSpec testTaskSpec = new TestTaskSpec(); testTaskSpec.setTestTaskId("testTaskId"); testTaskSpec.setDeviceIdentifier("deviceIdentifier"); taskQueue.offer(testTaskSpec); Mockito.when(testTaskService.getTestQueueCopy()).thenReturn(taskQueue); Mockito.when(deviceAgentManagementService.runTestTaskBySpec(testTaskSpec)).thenReturn(new JSONObject()); testTaskService.runTask(); Mockito.verify(deviceAgentManagementService, Mockito.times(1)).runTestTaskBySpec(testTaskSpec); Mockito.verify(testDataService, Mockito.times(1)).saveTestTaskData(Mockito.any(TestTask.class)); Mockito.verify(testTaskService, Mockito.times(1)).getTestQueueCopy(); Mockito.verify(testTaskService, Mockito.times(1)).isRunning.set(false); 
}
