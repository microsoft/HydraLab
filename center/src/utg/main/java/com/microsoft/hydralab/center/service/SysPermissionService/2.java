...
@Before
public void setUp() {
 permissionListMap = new ConcurrentHashMap<>(); sysPermissionService = new SysPermissionService(); sysPermissionService.sysPermissionRepository = sysPermissionRepository; sysPermissionService.permissionPermissionRelationRepository = permissionPermissionRelationRepository; sysPermissionService.permissionListMap = permissionListMap; 
}

@Test
public void testUpdatePermission() {
 SysPermission sysPermission = new SysPermission(); sysPermission.setPermissionId("1"); sysPermission.setPermissionType("type"); sysPermission.setPermissionContent("content"); sysPermission.setCreateTime(new Date()); sysPermission.setUpdateTime(new Date()); when(sysPermissionRepository.save(Mockito.any(SysPermission.class))).thenReturn(sysPermission); SysPermission result = sysPermissionService.updatePermission(sysPermission); assertEquals(sysPermission, result); 
}
