...
public class DeviceGroupServiceTest {
    ...
    public void testGetRelation() {
        ...
        when(deviceGroupRelationRepository.findById(id)).thenReturn(java.util.Optional.of(expectedRelation));

        DeviceGroupRelation actualRelation = deviceGroupService.getRelation(groupName, deviceSerial);

        assertEquals(expectedRelation, actualRelation);
        verify(deviceGroupRelationRepository, times(1)).findById(id);
    }
}