...
public void testUpdateAgentAliveStatus() {
 String agentId = "agent1"; String status = "online"; metricUtil.updateAgentAliveStatus(agentId, status); verify(meterRegistry, times(0)).gauge(anyString(), any(Tags.class), any(), any()); verifyNoMoreInteractions(meterRegistry); 
}
