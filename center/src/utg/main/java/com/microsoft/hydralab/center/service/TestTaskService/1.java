...
public void testIsQueueEmpty() {
 Queue<TestTaskSpec> taskQueue = Mockito.mock(Queue.class); Mockito.when(taskQueue.isEmpty()).thenReturn(true); testTaskService.taskQueue = taskQueue; Boolean result = testTaskService.isQueueEmpty(); Assert.assertTrue(result); 
}
