...
@Before
public void setup() {
 request = new MockHttpServletRequest(); response = new MockHttpServletResponse(); uploadedFile = Mockito.mock(MultipartFile.class); 
}

@Test
public void testUploadFile() {
 String fileUri = "testFile.txt"; String storageToken = "Bearer token"; InputStream inputStream = Mockito.mock(InputStream.class); Result expectedResult = Result.ok(fileUri); when(request.getHeader("Authorization")).thenReturn(storageToken); when(storageTokenManageService.validateAccessToken(storageToken)).thenReturn(true); when(uploadedFile.getInputStream()).thenReturn(inputStream); Result result = storageController.uploadFile(request, uploadedFile, fileUri); assertEquals(expectedResult, result); 
}
