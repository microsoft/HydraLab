...
public class DeviceGroupServiceTest {
    ...
    public void setup() {
        ...
        deviceGroupService.sysUserService = sysUserService;
        deviceGroupService.userTeamManagementService = userTeamManagementService;
    }
    ...
    public void testCheckGroupAuthorization() {
        ...
        boolean teamAdminRequired = true;

        DeviceGroup deviceGroup = new DeviceGroup();
        deviceGroup.setTeamId("testTeamId");

        Mockito.when(deviceGroupRepository.findById(groupName)).thenReturn(deviceGroup);
        Mockito.when(sysUserService.checkUserAdmin(requestor)).thenReturn(false);
        Mockito.when(userTeamManagementService.checkRequestorTeamAdmin(requestor, deviceGroup.getTeamId())).thenReturn(true);

        // Act
        boolean result = deviceGroupService.checkGroupAuthorization(requestor, groupName, teamAdminRequired);

        // Assert
        Mockito.verify(deviceGroupRepository).findById(groupName);
        Mockito.verify(sysUserService).checkUserAdmin(requestor);
        Mockito.verify(userTeamManagementService).checkRequestorTeamAdmin(requestor, deviceGroup.getTeamId());
        Mockito.verifyNoMoreInteractions(deviceGroupRepository, sysUserService, userTeamManagementService);
