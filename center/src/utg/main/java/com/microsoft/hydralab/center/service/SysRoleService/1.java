...
public void testCreateRole() {
 String roleName = "Test Role"; int authLevel = 1; SysRole sysRole = new SysRole(); sysRole.setRoleName(roleName); sysRole.setCreateTime(new Date()); sysRole.setUpdateTime(new Date()); sysRole.setAuthLevel(authLevel); when(sysRoleRepository.save(Mockito.any(SysRole.class))).thenReturn(sysRole); SysRole result = sysRoleService.createRole(roleName, authLevel); assertEquals(sysRole, result); 
}
