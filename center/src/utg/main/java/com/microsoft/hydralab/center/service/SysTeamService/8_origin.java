...
public class SysTeamServiceTest {
    ...
    public void testGetOrCreateDefaultTeam() {
        String defaultTeamName = "Default Team";
        SysTeam defaultTeam = new SysTeam();
        defaultTeam.setTeamName(defaultTeamName);
        defaultTeam.setCreateTime(new Date());
        defaultTeam.setUpdateTime(new Date());

        when(sysTeamRepository.findByTeamName(defaultTeamName)).thenReturn(Optional.of(defaultTeam));

        SysTeam result = sysTeamService.getOrCreateDefaultTeam(defaultTeamName);
